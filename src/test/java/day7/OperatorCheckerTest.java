...
class OperatorCheckerTest {
    ...
    void calibration() {
        assertEquals(
                dataProvider.getSampleResult(),
                OperationChecker.calibrationValue(dataProvider.getOperations(), List.of(new Add(), new Multiple()))
        );
    }

    @Test
    void calibrationWithConcat() {
        assertEquals(
                dataProvider.getSampleResultWithConcat(),
                OperationChecker.calibrationValue(dataProvider.getOperations(), List.of(new Add(), new Multiple(), new Concat()))
        );
    }
}